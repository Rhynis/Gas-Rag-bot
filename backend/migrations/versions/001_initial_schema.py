...
def upgrade() -> None:
    ...
    op.execute("COMMENT ON TABLE products IS 'Gas LPG products available for sale'")
    op.execute(
    ...
    op.execute("CREATE INDEX idx_orders_user_id ON orders(user_id) WHERE user_id IS NOT NULL")
    op.execute("CREATE INDEX idx_orders_customer_phone ON orders(customer_phone)")
    op.execute("CREATE INDEX idx_orders_status ON orders(status)")
    op.execute("CREATE INDEX idx_orders_source ON orders(source)")
    op.execute("CREATE INDEX idx_orders_created_at ON orders(created_at DESC)")
    op.execute(
        """
    ...
    op.execute("COMMENT ON COLUMN orders.user_id IS 'Nullable to support guest checkout'")
    op.execute(
    ...
    op.execute("COMMENT ON COLUMN orders.source IS 'website or chatbot'")
    op.execute(
    ...
    op.execute("COMMENT ON TABLE order_items IS 'Order line items with snapshot fields'")
    op.execute(
    ...
    op.execute("CREATE INDEX idx_conversations_session_id ON conversations(session_id)")
    ...
    op.execute("CREATE INDEX idx_messages_role ON messages(role)")
    op.execute("CREATE INDEX idx_messages_intent ON messages(intent) WHERE intent IS NOT NULL")
    op.execute(
        """
    ...
    op.execute("CREATE INDEX idx_kb_category ON knowledge_base(category)")
    op.execute("CREATE INDEX idx_kb_active ON knowledge_base(is_active)")
    op.execute(
        """
    ...
    op.execute("CREATE INDEX idx_kb_title_trgm ON knowledge_base USING gin (title gin_trgm_ops)")
    op.execute(
    ...
    op.execute(
        "CREATE POLICY products_public_active_select ON products FOR SELECT USING (is_active = TRUE)"
    )
    op.execute(
        "CREATE POLICY knowledge_base_public_active_select ON knowledge_base FOR SELECT USING (is_active = TRUE)"
    )
    op.execute("CREATE POLICY orders_guest_insert ON orders FOR INSERT WITH CHECK (TRUE)")
    op.execute(
        "CREATE POLICY conversations_guest_insert ON conversations FOR INSERT WITH CHECK (TRUE)"
    )
    op.execute("CREATE POLICY messages_insert ON messages FOR INSERT WITH CHECK (TRUE)")


def downgrade() -> None:
    """Reverse initial schema."""
    op.execute("DROP POLICY IF EXISTS messages_insert ON messages")
    op.execute("DROP POLICY IF EXISTS conversations_guest_insert ON conversations")
    op.execute("DROP POLICY IF EXISTS orders_guest_insert ON orders")
    op.execute("DROP POLICY IF EXISTS knowledge_base_public_active_select ON knowledge_base")
    op.execute("DROP POLICY IF EXISTS products_public_active_select ON products")

    op.execute("DROP FUNCTION IF EXISTS calculate_order_total(UUID)")
    op.execute("DROP FUNCTION IF EXISTS get_customer_orders(VARCHAR, INTEGER)")
    op.execute("DROP FUNCTION IF EXISTS match_documents(VECTOR(768), FLOAT, INT, TEXT)")

    op.execute("DROP TRIGGER IF EXISTS trigger_knowledge_base_updated_at ON knowledge_base")
    op.execute("DROP TRIGGER IF EXISTS trigger_conversations_updated_at ON conversations")
    op.execute("DROP TRIGGER IF EXISTS trigger_orders_updated_at ON orders")
    ...
    op.execute("DROP TRIGGER IF EXISTS trigger_set_order_number ON orders")
    op.execute("DROP FUNCTION IF EXISTS set_order_number()")
    op.execute("DROP FUNCTION IF EXISTS generate_order_number()")

    op.execute("DROP TABLE IF EXISTS knowledge_base")
    op.execute("DROP TABLE IF EXISTS messages")
    op.execute("ALTER TABLE orders DROP CONSTRAINT IF EXISTS fk_orders_referral_conversation")
